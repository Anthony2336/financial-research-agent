...
from fra.contracts import IntentRouter, ResearchCommand
from fra.domain import Intent, RouterDecision, SourceRef
from fra.execution import NoopResearchRunWriter, ResearchExecutionContext, current_research_context
from fra.graph.demo_models import (
    DEMO_TICKER,
...
from fra.mcp_server.tools import FetchRecentFilingsResponse
...
from fra.retrieval.indexing import EmbeddingIndexer, EmbeddingProvider
from fra.retrieval.ingest import ingest_fixture
from fra.retrieval.rerank import LazyFlashRankReranker
...
from fra.storage.database import (
...
class ResearchRuntime:
    ...
    def execute(
        ...
        from fra.graph.nodes import dispatch_skill_recipes
...
def build_p0_runtime(settings: Settings, *, ticker: str) -> ResearchRuntime:
    ...
    return _build_model_runtime(settings, ticker=ticker, thesis=True)
...
def _build_model_runtime(
    ...
    *,
    ...
    """Assemble the shared evidence services once; only model roles differ."""
    engine = create_engine(settings.database_url)
    _initialize_schema(engine)
    repository = FilingRepository(engine)
    normalized_ticker = ticker.strip().upper()
    ...
    if thesis:
        analyst = OpenAIThesisAnalystModel(model=settings.analyst_model, **model_options)
        dependencies = Dependencies(
            mcp_client=mcp_client,
            fast_model=OpenAIThesisFastModel(model=settings.fast_model, **model_options),
            analyst_model=analyst,
            thesis_repair_model=analyst,
            thesis_collector=collector,
            thesis_collection_policy=THESIS_COLLECTION_POLICY,
            web_evidence_validator=web_evidence_validator,
        )
    else:
        analyst = OpenAISkillAnalystModel(model=settings.analyst_model, **model_options)
        dependencies = Dependencies(
            mcp_client=mcp_client,
            fast_model=DeterministicDemoFastModel(),
            analyst_model=DeterministicDemoAnalystModel(),
            skill_planner=OpenAISkillPlannerModel(model=settings.fast_model, **model_options),
            skill_collector=collector,
            skill_analyst=analyst,
            skill_repair_model=analyst,
    ...
    def bind_run(context: ResearchExecutionContext) -> Dependencies:
        ...
        run_collector = EvidenceCollector(
            local_search=MCPFilingSearch(
                mcp_client,
                source_fetch_writer=context.run_repository,
                run_id=context.run_id,
        ...
        return replace(
            dependencies,
            trace_run=context.trace,
            thesis_collector=run_collector if thesis else None,
            skill_collector=None if thesis else run_collector,
        )
    ...
    return ResearchRuntime(
        ...
        execution_note=(
            ...
            if web_search is not None
                ...
                "production thesis (allowlisted web fallback unavailable; local evidence only)"
                if thesis else "allowlisted web fallback unavailable; local evidence only"
            )
        ),
...
def build_market_runtime(
    ...
    *,
    ...
    server = FastMCP("Financial-Research-Agent-market")
