"""Command-line entry points for the financial evidence agent."""
...
from financial_evidence_agent.market_data.models import MarketDataError
from financial_evidence_agent.retrieval.indexing import EmbeddingModelUnavailableError
...
def ingest(
    ...
    from financial_evidence_agent.retrieval.ingest import (
    ...
    from financial_evidence_agent.retrieval.xbrl import HttpCompanyFactsGateway, XbrlError
    from financial_evidence_agent.storage.cache import build_cache
...
def evaluate(
    ...
    from financial_evidence_agent.evals import (
...
def _legacy_eval_failed(summary: object) -> bool:
    if getattr(summary, "passed_count") != getattr(summary, "case_count"):
        return True
    if getattr(summary, "p1_passed_count") != getattr(summary, "p1_case_count"):
        return True
    metrics = getattr(summary, "p1_metrics", None)
    return metrics is not None and any(
        getattr(metrics, name) != 0 for name in ("source_policy_violations", "budget_violations")
    )


def _p2_eval_failed(summary: object) -> bool:
    if getattr(summary, "passed_count") != getattr(summary, "case_count"):
        return True
