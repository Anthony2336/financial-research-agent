...
from financial_evidence_agent.context import BudgetAuthority, BudgetGate, MemoryHint
...
from financial_evidence_agent.market_data.models import MarketDataBundle
from financial_evidence_agent.memory.research import ResearchMemorySearch
from financial_evidence_agent.memory.session import SessionMemoryRepository
from financial_evidence_agent.observability import NoopTraceSink, TraceRun, TraceSink
from financial_evidence_agent.reporting.guard import GuardedMemo
from financial_evidence_agent.reporting.market_guard import GuardedMarketReport
...
from financial_evidence_agent.skills.schemas import GuardedSkillMemo, SkillResearchMemo
...
def persisted_claim(claim: object, *, ticker: str) -> PersistedClaim:
    """Encode one retained claim without flattening filing and web provenance."""
    kind = getattr(claim, "kind")
    confidence = getattr(claim, "confidence")
    return PersistedClaim(
        ...
        source_refs=[
            *(
                ...
                for source_id in getattr(claim, "evidence_chunk_ids")
            ),
            *(
                SourceRef(
                    ticker=ticker,
                ...
                for source_id in getattr(claim, "web_evidence_ids")
            ),
        ],
        guard_status="retained",
    )
